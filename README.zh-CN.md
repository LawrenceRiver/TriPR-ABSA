...
<details>
<summary>模块消融与限制</summary>

所有消融版本使用相同的 TextGT-BERT 检查点和测试集。改动、修正和新增错误均为
检查点平均值。

| 策略 | 准确率 | Macro-F1 | F1 变化 | 改动 | 修正 | 新增错误 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 事实 | 0.8406 | 0.7522 | +0.0042 | 4.33 | 3.33 | 0.67 |
| 比较 | 0.8394 | 0.7499 | +0.0019 | 1.33 | 1.33 | 0.00 |
| 强度 | 0.8388 | 0.7488 | +0.0008 | 2.67 | 1.33 | 0.67 |
| 事实 + 比较 | 0.8418 | 0.7541 | +0.0060 | 5.67 | 4.67 | 0.67 |
| 全部残差 | 0.8424 | 0.7547 | +0.0067 | 8.00 | 6.00 | 1.33 |

事实分支是效果最强的单个模块。比较分支改动的样本较少，报告均值中没有新增错误。
三个分支一起使用时综合结果最好，但仍有过度修正的情况。

目前报告的 Macro-F1 增益为 `+0.0067`，跨领域结果并不稳定，方法还依赖人工线索
和固定残差权重。发布时没有重新运行完整 GPU 实验。完整主结果、基线错误分布、
数值差异和限制说明见 [docs/results.md](docs/results.md)。
